...
var.title <- c(pm2.5="expression('Coefficients for'~PM[2.5]~'from Daily Models')",
               rh="'Coefficients for Relative Humidity from Daily Models'",
               temp="'Coefficients for Temperature from Daily Models'",
               cnst="'Constant Terms from Daily Models'")
...
for (var in c('pm2.5','rh','temp','cnst')) {
  eval(parse(text=paste0("form <- ",var," ~ day + mean.pm2.5 + mean.temp")))
  coefs <- summary(lm(form,data=daily.model))$coefficients
  writeLines(paste0('\n',headers[var],'\n'))
  print(coefs)
}
```